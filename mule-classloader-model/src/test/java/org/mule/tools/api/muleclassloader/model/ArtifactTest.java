...
class ArtifactTest {
  ...
  void setNewArtifactURIWindowsRelativePathTest() throws URISyntaxException {
    ...
    assertThat(newArtifact.getUri())
        .as("Relative path is not the expected")
        .isEqualTo(EXPECTED_URI);
  }
  ...
  @Test
  void getFormattedMavenDirectoryTest(@TempDir Path outputFolder) {
    File actual = artifact.getFormattedMavenDirectory(outputFolder.getRoot().toFile());
    File expected = new File(outputFolder.getRoot().toFile(), OUTPUT_DIRECTORY);
    assertThat(actual.getAbsolutePath())
        .as("Actual formatted output directory is not the expected")
  ...
  void getFormattedArtifactFileNameTest() {
    ...
    assertThat(actual)
        .as("Formatted file name is different from the expected")
        .isEqualTo(expected);
  }
  ...
  @Test
  void artifactsAreNotSharedByDefault() {
    artifact = new Artifact(artifactCoordinates, EXPECTED_URI);
    assertThat(artifact.isShared()).isFalse();
  }

  @Test
  void artifactsWithDifferentClassifierAreNotEqual() {
