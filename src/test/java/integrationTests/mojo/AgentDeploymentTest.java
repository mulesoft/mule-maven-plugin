...
import integrationTests.mojo.environmentSetup.AgentSetup;
...
public class AgentDeploymentTest {
  ...
  private void verifyDeployment(boolean isDeployed) throws InterruptedException, IOException, VerificationException {
    ...
    for (int i = 0; i < 10 && !deployed; ++i, Thread.sleep(1000)) {
      deployed = FileUtils.fileExists(agentSetup.getAnchorFilePath());
    }
    assertThat("Failed to deploy", deployed, is(isDeployed));
