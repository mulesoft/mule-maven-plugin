...
public class MuleArtifactArchiverLauncher {
  ...
  private static final Option ARTIFACT_TYPE_OPTION = Option.builder().argName(ARTIFACT_CONTENT_TYPE_ARGUMENT_NAME)
      .longOpt(ARTIFACT_CONTENT_TYPE).hasArg().desc("define if package contains binaries and/or sources").build();
  private static final Option TARGET_FOLDER_OPTION = Option.builder().argName(TARGET_FOLDER_ARGUMENT_NAME).longOpt(TARGET_FOLDER)
      .hasArg().desc("path of source package structure root folder").build();
  private static final Option PACKAGE_NAME_OPTION = Option.builder().argName(PACKAGE_NAME_ARGUMENT_NAME).longOpt(PACKAGE_NAME)
      .hasArg().desc("file name for generated file").required().build();
