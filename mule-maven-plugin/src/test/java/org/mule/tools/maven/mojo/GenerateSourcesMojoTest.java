...
public class GenerateSourcesMojoTest extends AbstractMuleMojoTest {
    ...
    public void createDescriptorFilesContentTest() throws IOException {
        File pomDestinationFolder = new File(temporaryFolder.getRoot().getAbsolutePath(), META_INF + File.separator + MAVEN + File.separator + GROUP_ID + File.separator + ARTIFACT_ID);
        pomDestinationFolder.mkdirs();
        File propertiesDestinationFolder = new File(temporaryFolder.getRoot().getAbsolutePath(), META_INF + File.separator + MULE_ARTIFACT);
        ...
        mojo.descriptorManager = new DescriptorManager(projectMock, temporaryFolder.getRoot());
        mojo.project = projectMock;
        ...
        List<File> actualFilesInPomDestinationFolder = Arrays.asList(pomDestinationFolder.listFiles()).stream().filter(file -> file.isFile()).collect(Collectors.toList());
        assertThat("The pom destination folder does not contains the expected file", actualFilesInPomDestinationFolder.get(0).getName(), equalTo(POM_XML));

        assertThat("There should be 2 files in the properties destination folder", propertiesDestinationFolder.listFiles().length, equalTo(2));
        List<File> actualFilesInPropertiesDestinationFolder = Arrays.asList(propertiesDestinationFolder.listFiles()).stream().filter(file -> file.isFile()).collect(Collectors.toList());
        assertThat("The properties destination folder does not contains the expected files", actualFilesInPropertiesDestinationFolder.stream().map(file -> file.getName()).collect(Collectors.toList()), containsInAnyOrder(MULE_APP_PROPERTIES, MULE_DEPLOY_PROPERTIES));
    }
}